...
namespace Carotaa.Code
{
	public static class DoTweenModuleAnimationClip
	{
		...
		public static Tweener Internal_DoAnimationClip (List<PropertyBridge> bridges, float time, bool reset = true)
		{
			var progress = 0f;
			void Setter(float x)
			{
		...
		public abstract class PropertyBridge
		{
			...
			public string Name => _curve.Name;
				...
				ShareBufferProvider bufferProvider, out PropertyBridge setter)
			{
				...
				if (string.IsNullOrEmpty(curve.PropertyName)) return false;
