...
namespace Carotaa.Code
{
    public static class FileUtility
    {
        ...
        public static string PersistentDataPath => _dataPath ?? (_dataPath = Application.persistentDataPath);
        ...
        public static void ReadALlTextAsyncWithCallback(string filePath, Encoding encoding, Action<string> onComplete)
        {
            ...
            task.ContinueWith(prv => { Loom.QueueOnMainThread(() => onComplete.Invoke(prv.Result)); });
        }
        ...
        public static void ReadAllBytesAsyncWithCallback(string filePath, Action<byte[]> onComplete)
        {
            ...
            task.ContinueWith(x => { Loom.QueueOnMainThread(() => onComplete.Invoke(x.Result)); });
        }
        ...
        private static void OnTaskComplete(string path, Task task)
        {
            lock (s_fileTasks)
            {
                if (s_fileTasks.ContainsKey(path) && s_fileTasks[path] == task) s_fileTasks.Remove(path);
            }
        }
